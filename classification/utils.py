...
def one_cycle_scheduler(base_value, final_value, epochs, niter_per_ep, warmup_epochs=0,
                     start_warmup_value=0, warmup_steps=-1):
    iters = np.arange(epochs * niter_per_ep)
    no_iters = len(iters)
    schedule = np.array(np.interp(iters, [0, no_iters*2//5, no_iters*4//5, no_iters], 
                                    [0, base_value, base_value/20.0, 0]))

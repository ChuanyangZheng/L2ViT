...
class L2ViT(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000, mlp_ratio=[4., 4., 4., 4., ],
        ...
        for i in range(4):
            stage = CustomSequential(
                *[build_block(block_type, index=j, dim=dims[i], drop_path=dp_rates[cur + j], mlp_ratio=mlp_ratio[i],
                              ...
                              ffn_conv=ffn_conv, rffn_conv=rffn_conv,
        ...
        for i_layer in out_indices:
            layer = build_norm(norm, dims[i_layer])
    ...
    def init_weights(self, pretrained=None):
        ...
        Args:
            pretrained (str, optional): Path to pre-trained weights.
                Defaults to None.
        """
        pretrained = self.pretrained or pretrained
    ...
    def forward_features(self, x):
        ...
        for i in range(4):
            ...
            if i in self.out_indices:
                ...
                x_out = norm_layer(x)
                H, W = size
                out = x_out.view(-1, H, W, self.dims[i]).permute(0,3,1,2).contiguous()
                outs.append(out)
